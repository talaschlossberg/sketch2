...
How the tracing works (all in the browser, nothing is uploaded): the photo is shrunk, smoothed with an edge-preserving bilateral filter, its colors are clustered with k-means in Lab space, tiny fragments are merged into their neighbours, and each region's boundary is traced into an SVG path (RDP simplification + Chaikin smoothing). Sounds are generated with the Web Audio API — each swatch is a note on an A-minor pentatonic scale, and bigger spaces ring a little longer.
